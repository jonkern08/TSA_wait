...
function getETDayRange(dateStr: string): { start: string; end: string } {
  // Determine ET UTC offset by checking what ET hour noon UTC maps to
  ...
  // etHourAtNoon = 7 → EST (UTC-5), etHourAtNoon = 8 → EDT (UTC-4)
  const behindUTC = 12 - etHourAtNoon; // 5 for EST, 4 for EDT
