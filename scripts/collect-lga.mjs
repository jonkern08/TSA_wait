  ...
  const extractRes = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `Extract LaGuardia Airport (LGA) TSA security wait times. Return ONLY this JSON format:

{"A":{"general":NUMBER_OR_NULL,"precheck":NUMBER_OR_NULL,"closed":BOOLEAN},"B":{"general":NUMBER_OR_NULL,"precheck":NUMBER_OR_NULL,"closed":BOOLEAN},"C":{"general":NUMBER_OR_NULL,"precheck":NUMBER_OR_NULL,"closed":BOOLEAN},"D":{"general":NUMBER_OR_NULL,"precheck":NUMBER_OR_NULL,"closed":BOOLEAN}}
...
- LaGuardia has 4 terminals: A, B, C, D
...
${rawText}`,
  ...
  const insertRes = await fetch(`${process.env.SUPABASE_URL}/rest/v1/lga_wait_times`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
